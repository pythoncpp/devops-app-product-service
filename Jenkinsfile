node {
    ...
    stage('SCM') {
      git 'https://github.com/pythoncpp/devops-app-product-service.git'
    }

    // building the docker image
    stage('Build Docker image') {
      sh 'docker image build . -t pythoncpp/devops-app-product-service'
    }
    ...
    stage('Docker hub push') {
      withCredentials([usernamePassword(credentialsId: 'docker_hub', passwordVariable: 'password', usernameVariable: 'username')]) {
          ...
          sh 'docker image push pythoncpp/devops-app-product-service'

          // delete the image from local machine
          sh 'docker image rm pythoncpp/devops-app-product-service'
      }
    }
